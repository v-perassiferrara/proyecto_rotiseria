...
class Notificaciones(Resource):
    ...
    def get(self):
        ...
        if claims.get("rol") == "admin":
            ...
            notificaciones = db.session.query(Notificacion_db).filter(
                Notificacion_db.fk_id_usuario == current_identity
            )
        ...
        if request.args.get("fecha_inicio") and request.args.get("fecha_fin"):
            ...
            notificaciones = notificaciones.filter(
                Notificacion_db.fecha >= fecha_inicio,
                Notificacion_db.fecha <= fecha_fin,
            )
        ...
        return jsonify(
            {
                "notificaciones": [
                    notificacion.to_json() for notificacion in notificaciones
                ],
    ...
    def post(self):
        try:
            ...
            notificacion = Notificacion_db.from_json(data)
            ...
            return {"error": "Error al guardar la notificación"}, 500
...
class Notificacion(Resource):
    ...
    def get(self, id):
        ...
        if (
            current_identity == notificacion.fk_id_usuario
            or claims.get("rol") == "admin"
        ):
            return notificacion.to_json(), 201
